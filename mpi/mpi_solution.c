    ...
    for (int benchmark = 0; benchmark < 25; benchmark++) {
        ...
        if (thread_num == 0) {
            ...
            for (int i = 1; i < thread_count; i++) {
                ...
                memcpy(&boundary[1], &input_data->matrix[MATRIX_POSITION(start_index - 1, 0, input_data)],
                       sizeof(int) * column_count);
                ...
                if (i == thread_count - 1) {
                    ...
                    memcpy(&boundary[1], &input_data->matrix[MATRIX_POSITION(end_index, 0, input_data)],
                           sizeof(int) * column_count);
        ...
        if (thread_num < row_count) {
            ...
            if (thread_num > 0) {
                ...
                if (thread_num == thread_count - 1) {
                    ...
                    memcpy(&bottom[1], &input_data->matrix[MATRIX_POSITION(abs_end_index, 0, input_data)],
                           sizeof(int) * column_count);
            ...
            for (int iteration = 0; iteration < stencil.iteration_count; iteration++) {
                ...
                if (thread_num > 0) {
                    MPI_Irecv(&data->top[1], column_count, MPI_INT, thread_num - 1, 0, MPI_COMM_WORLD,
                              &requests[count]);
                    count++;
                }

                if (thread_num + 1 < thread_count) {
                    MPI_Irecv(&data->bottom[1], column_count, MPI_INT, thread_num + 1, 0, MPI_COMM_WORLD,
                              &requests[count]);
                    count++;
                }
