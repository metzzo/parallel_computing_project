...
void test_constant_matrix(int row_count, int column_count, int thread_count, int iteration_count) {
    ...
    for (int row = 0; row < row_count; row++) {
        for (int column = 0; column < column_count; column++) {
            input_matrix_1[row * column_count + column] = input_matrix_2[row * column_count + column] = input_matrix_3[row * column_count + column] = 1;
    ...
    check_equal(&data3, &expected_datas);
